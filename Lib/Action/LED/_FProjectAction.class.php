...
{
public function getAllFProject() {
	...
	public function updateFProjectState()
	{	
		$jsonInput = file_get_contents("php://input"); 
		$jsonInput=$this->checkUTF8($jsonInput);
		$decodedUser=json_decode($jsonInput);
		...
		// 下面将数据添加到数据库中
		$sqlExecute = "update FProject set Pstate = '$Pstate'  where PID = '$PID';";
	...
	public function getFProjectCount()
	{
		$jsonInput = file_get_contents("php://input"); 
		$jsonInput=$this->checkUTF8($jsonInput);
		$decodedUser=json_decode($jsonInput);
