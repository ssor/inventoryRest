...
class OrderAction extends Action
{
	public function deleteOrders()
	{
		...
		if (count($decodedOrders)>0) {
			...
			for($i=0;$i<count($decodedOrders);$i++)
			{
				...
				$sqlExecute = "delete from tbOrder_barcode where productName = '$productName';";
				$M = new Model();
	...
	public function addOrder()
	{
		...
		$productName = $decodedOrder->productName;
		$productQuantity = $decodedOrder->quantity;
		$sqlExecute = "insert into tbOrder_barcode(productName,productQuantity) values('$productName',$productQuantity);";
		$M = new Model();
		$r = $M->execute($sqlExecute);
		require_once('class.Order.php');
		$order = new Order(
